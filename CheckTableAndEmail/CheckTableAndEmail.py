    ...
    def execute(self):
        ...
        if DKDATA_KEY in self.postgres.config_dict and \
            ...
            if self.config_dict[AC_EMAIL_SEND_TEST][AC_EMAIL_TEST_LOGIC] == AC_EMAIL_TEST_GT:
                ...
                send_it = False
            self.set_progress('variables', json.dumps(sub_dict, indent=4))
...
def main(parser=None):
    ...
    if ac_check_table.vaild_config() is True and \
        ...
        if ac_check_table.parse_command_line(parser) is True:
            ...
            try:
                shutil.copyfile(log_filename, ac_check_table.inside_log_file)
            except IOError:
                ACLogger.log_and_print_error('CheckTableAndEmail: unable to move log file from %s to %s' %
                                              (log_filename, ac_check_table.inside_log_file))
        else:
